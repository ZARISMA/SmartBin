"""
hexabin/control.py — unified runnable entry point.

Reads ``HEXABIN_CAMERA_MODE`` (pipeline) and ``HEXABIN_STRATEGY``
...
def _parse_cli() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="HexaBin AI — unified edge runner",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    p.add_argument(
        "--pipeline",
        choices=VALID_PIPELINES,
        help="Camera pipeline (overrides HEXABIN_CAMERA_MODE)",
    )
    p.add_argument(
        "--strategy",
        choices=VALID_STRATEGIES,
        help="Strategy for dual-OAK pipeline (overrides HEXABIN_STRATEGY)",
    )
...
def _resolve_pipeline() -> str:
    ...
    env = os.environ.get("HEXABIN_CAMERA_MODE", "").strip().lower()
...
def _resolve_strategy() -> str:
    ...
    env = os.environ.get("HEXABIN_STRATEGY", "").strip().lower()
...
def main() -> None:
    ...
    logger.info("hexabin-run: pipeline=%s", pipeline)
    ...
    try:
        ...
        logger.exception("hexabin-run crashed: %s", exc)
        exit_code = 1
