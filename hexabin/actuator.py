"""
hexabin/actuator.py — pluggable bin-module actuation.
...
drops in later:
  ...
  3. Set ``HEXABIN_ACTUATOR=gpio`` on the edge device.

The category → module mapping comes from ``HEXABIN_MODULE_MAP`` (JSON) and
...
class NullActuator:
    """Discard actuation commands entirely (HEXABIN_ACTUATOR=none)."""
    ...
    if _instance is None:
        with _instance_lock:
            if _instance is None:
                ...
                if cls is None:
                    logger.warning("Unknown HEXABIN_ACTUATOR %r — using 'log'.", ACTUATOR)
                    cls = LogActuator
                _instance = cls()
    return _instance
