"""Tests for crop_sides() in smartwaste/cameraraspberry.py."""
...
# Mock picamera2 before importing cameraraspberry
sys.modules['picamera2'] = MagicMock()

from smartwaste.cameraraspberry import crop_sides
...
class TestNoCrop:
    ...
    def test_negative_percent_returns_same_shape(self):
        frame = np.zeros((100, 200, 3), dtype=np.uint8)
        assert crop_sides(frame, -5.0).shape == frame.shape
...
class TestCropGeometry:
    ...
    def test_25_percent_on_200px_wide(self):
        ...
        result = crop_sides(frame, 25.0)
        assert result.shape[1] == 100

    def test_50_percent_yields_zero_width(self):
        frame = np.zeros((10, 100, 3), dtype=np.uint8)
        result = crop_sides(frame, 50.0)
    ...
    def test_odd_width_floor_truncation(self):
        ...
        assert crop_sides(frame, 20.0).shape[1] == right - left

    def test_large_hd_frame(self):
        frame = np.zeros((1080, 1920, 3), dtype=np.uint8)
        left = int(1920 * 0.20)
        right = int(1920 * 0.80)
        assert crop_sides(frame, 20.0).shape == (1080, right - left, 3)
...
class TestGrayscaleFrames:
    ...
    def test_grayscale_height_preserved(self):
        frame = np.zeros((100, 200), dtype=np.uint8)
        assert crop_sides(frame, 20.0).shape[0] == 100
