"""
smartwaste/llm.py — pluggable LLM classification backends.

Backends (selected by SMARTWASTE_LLM_BACKEND):
...
* ``cascade``  — LM Studio first; escalate the same image to Gemini when the
  ...
  SMARTWASTE_CONFIDENCE_THRESHOLD.
    ...
    def classify(self, img_bytes: bytes) -> ClassificationResult:
        ...
        if not raw:
            ...
            raise LLMError(
                ...
                f"if 'length', raise SMARTWASTE_LMSTUDIO_MAX_TOKENS."
            )
...
def build_backend(name: str | None = None) -> LLMBackend:
    """Build the backend named by *name* (default: SMARTWASTE_LLM_BACKEND)."""
