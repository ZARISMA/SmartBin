...
Uses three OAK-D hardware sensors to decide when the bin is occupied,
then fires one Gemini API call for waste classification:
...
def _draw_overlay(
    ...
    cv2.putText(frame,
                f"Smart Waste AI (OAK-D Native)   |   {state_str}",
...
def _tick(
    ...
    if state == OakState.DETECTED:
        if votes.rgb_frame is not None and app_state.start_classify():
            img_bytes = encode_frame(votes.rgb_frame)
            launch_classify(img_bytes, votes.rgb_frame.copy(), app_state)
...
def main() -> None:
    ...
    p = argparse.ArgumentParser(
        description="SmartWaste AI — OAK-D Native mode (single camera)",
    ...
    try:
        ...
        cv2.resizeWindow(OAK_WINDOW, OAK_DISPLAY_W, OAK_DISPLAY_H)
    ...
    with dai.Device(infos[0]) as device:
        ...
        logger.info("Starting Smart Waste AI (OAK-D Native)")

        try:
            while True:
                # ── Drain all queues every iteration ──────────────────────────
                votes = detector.update()
                if votes.rgb_frame is not None:
                    last_votes = votes

                # ── Display ───────────────────────────────────────────────────
                if last_votes.rgb_frame is not None:
                    disp = cv2.resize(
                        last_votes.rgb_frame,
                        (OAK_DISPLAY_W, OAK_DISPLAY_H),
                        interpolation=cv2.INTER_AREA,
                    )
                    _draw_overlay(disp, oak_state, last_votes, app_state,
                                  detector, calib_pct[0])
